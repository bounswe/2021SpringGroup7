...
const getLikeStory = useMutation(params => SERVICE.getLikes({params, token}), {
    onSuccess(response) {
  ...
  return (
    <HStack
      ...
      {!liked ? (
        ...
        <Icon name={'heart'} onPress={()=>like()} color="red" solid size={20} onLongPress={()=>getLike()} />
      )}
      <Modal isOpen={showLikeModal} onClose={() => setShowLikeModal(false)}>
        <Modal.Content maxWidth="400px">
          ...
          <Modal.Body>
            {likers.map((item)=>{
              return <HStack ml={3} mt={4} space={1} alignItems="center">
      <Avatar
        size="sm"
        bg={colors[rand]}
        ml={-2}
